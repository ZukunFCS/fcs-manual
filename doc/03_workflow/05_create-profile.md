...
本機能は、Galleryに登録されたProfileを効率的に管理し特定の表情や設定をスムーズに切り替えるための機能です。

1.Editorウィンドウの【▶Tags】を開き、テキストボックスにタグの名前を入力、【＋】を押します。  
...
**使用例**  
 ...
 Batch EditでGallery現在表示されているものや特定のタグが付いたProfileに対して、「一括で使用・不使用（ON/OFF）」を切り替えることができます。  

1.PropertyでどのRegionの使用・不使用（ON/OFF）を設定するか選択します。
...
② 精度の向上（ノイズの除去）  
「カメラの向きが異なるプロファイル」などにタグを付けて一括で「不使用」に設定することで、  
現在の環境に最適なプロファイルのみが適用され、表情の認識精度を高めることができます。  
